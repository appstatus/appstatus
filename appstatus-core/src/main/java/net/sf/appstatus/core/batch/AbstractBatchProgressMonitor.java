...
public abstract class AbstractBatchProgressMonitor implements IBatchProgressMonitorExt {
    ...
    /**
     * Do not show estimated end date if more than 1 year.
     */
    private static Date maxDate = new Date(System.currentTimeMillis() + 1000 * 60 * 60 * 24 * 365);
    ...
    public AbstractBatchProgressMonitor(String executionId, IBatch batch) {
        ...
        getLogger().info("[{}] {}: Init progress monitoring (id: {})", new Object[] { this.batch.getGroup(), batch.getName(), batch.getUuid() });
        touch();
    }
    ...
    public void beginTask(String name, String description, int totalWork) {
        ...
        getLogger().info("[{}] {}: Begin {} ({}), steps : {}", new Object[] { getBatch().getGroup(), getBatch().getName(), name, description, String.valueOf(totalWork) });
        touch();
    ...
    public void done() {
        if (parent != null) {
            endTask(true);
            getLogger().info("[{}] {}: End {}, {} ms", new Object[] { getBatch().getGroup(), getBatch().getName(), name, System.currentTimeMillis() - startTime });
        } else {
            endBatch(true);
            getLogger().info("[{}] {}: End batch, {} ms", new Object[] { getBatch().getGroup(), getBatch().getName(), System.currentTimeMillis() - startTime });
    ...
    public void done(String customStatus) {
        getLogger().info("[{}] {}: Ended with custom status {}", new Object[] { getBatch().getGroup(), getBatch().getName(), customStatus });
        done();
    }
    ...
    public void fail(String reason, Throwable t) {
        ...
        getLogger().error("[{}] {}: FAILED ({}) , duration: {} ms", this.batch.getGroup(), batch.getName(), reason, String.valueOf(endTime - startTime), t);
        touch();
    }
    ...
    public void reject(String itemId, String reason, Throwable e) {
        ...
        getLogger().warn("[{}] {}: {} rejected {} ({})", getBatch().getGroup(), getBatch().getName(), name, itemId, reason, e);
    ...
    public void setCurrentItem(Object item) {
        ...
        if (isLoggable(lastWriteTimestamp)) {
            ...
            getLogger().info("[{}] {}: {} working on {} (#{})", new Object[] { getBatch().getGroup(), getBatch().getName(), actionName, item, itemCount });
        }
        touch();
