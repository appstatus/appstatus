...
public class ServicesPage extends AbstractPage {
    ...
    public void doGet(AppStatus status, HttpServletRequest req, HttpServletResponse resp)
        ...
        if (HtmlUtils.generateBeginTable(os, services.size())) {
            ...
            HtmlUtils.generateHeaders(os, "", "Category", "Name", "Hits", "Cache", "Running", "min", "max", "avg",
                    "min (cached)", "max (cached)", "avg (cached");

            for (IService service : services) {
                HtmlUtils.generateRow(os, Icons.STATUS_JOB, service.getGroup(), service.getName(), service.getHits(),
                        service.getCacheHits(), service.getRunning(), service.getMinResponseTime(),
                        service.getMaxResponseTime(), Math.round(service.getAvgResponseTime()),
