...
public class BatchDao {
    ...
    private List<BdBatch> fetchBdBatch(final int max, String[] status) {
        ...
        SqlRowSet srs = this.jdbcTemplate.queryForRowSet(getSqlWithListOfStatus(status), new Object[] { max });
    ...
    /**
     * Getting the formated query with a list of status
     * 
     * @param status
     * @return String
     */
    private String getSqlWithListOfStatus(String[] status) {
        for (int i = 0; i < status.length; ++i) {
            status[i] = String.format("'%s'", status[i]);
        }

        return String.format(getSql(BATCH_FETCH), StringUtils.join(status, ","));
    }
