...
public class BatchDao {
    ...
    private List<BdBatch> fetchBdBatch(final int max, String[] status) {
        SqlRowSet srs = this.jdbcTemplate.queryForRowSet(insertParametersFromList(getSql(BATCH_FETCH), status), new Object[] { max });
    ...
    public List<BdBatch> fetchFinished(final int max) {
        return fetchBdBatch(max, new String[] { IBatch.STATUS_SUCCESS, IBatch.STATUS_FAILURE });
    }
    ...
    protected String getSql(int query) {
        ...
        case BATCH_UPDATE:
            return "UPDATE " + tableName + " set ITEM = ?, CURRENT_TASK = ?, END_DATE=?, GROUP_BATCH=?,  ITEMCOUNT=?, " + "LAST_MSG = ?, UPDATED=?, NAME_BATCH=?, PROGRESS = ?, REJECT = ?, STATUS=?, " + "SUCCESS=?  WHERE  UUID_BATCH=?";
        ...
        case BATCH_INSERT:
            return "INSERT into " + tableName + " (UUID_BATCH,GROUP_BATCH,NAME_BATCH,START_DATE,STATUS,ITEMCOUNT) values (?,?,?,?,?,0)";
        ...
        case BATCH_FETCH_BY_NAME:
            return "SELECT UUID_BATCH, ITEM, CURRENT_TASK, END_DATE, GROUP_BATCH, ITEMCOUNT, LAST_MSG, UPDATED," + " NAME_BATCH, PROGRESS, REJECT, START_DATE, STATUS,SUCCESS FROM "//
                    + tableName + " WHERE  GROUP_BATCH = ? AND NAME_BATCH = ? AND STATUS IN ( %s ) ORDER BY UPDATED DESC LIMIT ? ";
    ...
    public BdBatch save(BdBatch bdBatch) {
        ...
        logger.debug("PARAMETERS UUID BATCH:{} NAME: {} GROUP: {}", bdBatch.getUuid(), bdBatch.getName(), bdBatch.getGroup());
    ...
    public void update(BdBatch bdBatch) {
        ...
        Object[] parameters = new Object[] { bdBatch.getCurrentItem(), bdBatch.getCurrentTask(), bdBatch.getEndDate(), bdBatch.getGroup(), bdBatch.getItemCount(), bdBatch.getLastMessage(), bdBatch.getLastUpdate(), bdBatch.getName(),
                bdBatch.getProgress(), bdBatch.getReject(), bdBatch.getStatus(), bdBatch.getSuccess(), bdBatch.getUuid() };
        this.jdbcTemplate.update(getSql(BATCH_UPDATE), parameters);
    }

    @Transactional(readOnly = true)
    public List<BdBatch> fetch(String group, String name, int max) {
        return fetchBdBatch(group, name, max, new String[] { IBatch.STATUS_SUCCESS, IBatch.STATUS_FAILURE });
    }
