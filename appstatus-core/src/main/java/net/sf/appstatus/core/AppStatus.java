...
public class AppStatus {
    ...
    private ICheckResult createCheckResultFromException(ICheck c, Exception e) {
        ...
        return new CheckResultBuilder().from(c).code(ICheckResult.ERROR).fatal().description("Check failed with exception: " + e.getClass().getCanonicalName() + " " + e.getMessage()).build();

    }

    /**
     * @return the batchExprAdapter
     */
    public IBatchExprAdapter getBatchExprAdapter() {
        return batchExprAdapter;
    }
    ...
    private Object getClassInstance(String className) {
        ...
        if (obj == null) {
            try {
                ...
                logger.warn("Class {} loaded using a deprecated method. Please report to http://sourceforge.net/apps/mantisbt/appstatus/login_select_proj_page.php?ref=bug_report_page.php", className);
            } catch (ClassNotFoundException e) {
                logger.warn("Class {} not found ", className, e);
            } catch (InstantiationException e) {
                logger.warn("Cannot instanciate {} ", className, e);
    ...
    /**
     * @param batchExprAdapter
     *            the batchExprAdapter to set
     */
